...
function left() {
    ...
    if (handles[selected][0] < 0)
        handles[selected][0] = 0;
...
function right() {
    if (selected === 0)
        return;
    handles[selected][0] += getPitch();
    if (width < handles[selected][0])
        handles[selected][0] = width;
    navigator.vibrate(100);
    setPlayMode(true);
    log.push(JSON.parse(JSON.stringify(handles)));
}
function up() {
    ...
    if (handles[selected][1] < 0)
        handles[selected][1] = 0;
    navigator.vibrate(100);
    setPlayMode(true);
    log.push(JSON.parse(JSON.stringify(handles)));
}
function down() {
    if (selected === 0 || selected === 1)
        return;
    handles[selected][1] += getPitch();
    if (height < handles[selected][1])
        handles[selected][1] = height;
    navigator.vibrate(100);
    setPlayMode(true);
    log.push(JSON.parse(JSON.stringify(handles)));
}
